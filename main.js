...
function _address(resp) {
    var address = JSON.parse(resp.result);
    ajax("/v1/user/accountstate", {"address": address}, (result) => {
        var balance = result.result.balance;
        var weiToNas = parseFloat(balance) / (10 ** 18)
        NASToUSD().done(function(data) {
            NASpriceUSD = parseFloat(data[0].price_usd)
            var totalBalance = weiToNas * NASpriceUSD
            $('.bal').text(totalBalance.toFixed(4));
        })
...
function ajax(path, data, _successCb, _errorCb) {
    $.ajax({
        type: 'POST',
        url: rpcEndpoint + path,
...
function NASToUSD() {
    var coinAPI = "https://api.coinmarketcap.com/v1/ticker/nebulas-token/";
    return $.getJSON(coinAPI, {
        option: "none yet"
    })
}
